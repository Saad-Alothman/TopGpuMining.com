...
namespace CreaDev.Framework.Core.Imaging
{
    public class Image
    {
        public Bitmap DrawFilledRectangle(int x, int y,string argbColor="#FFFFFFFF")
        {

            string color = argbColor.TrimStart('#');
            string Astr = color.Substring(0, 2);
            string Rstr = color.Substring(2, 2);
            string Gstr= color.Substring(4, 2);
            string Bstr = color.Substring(6, 2);
            int a= int.Parse(Astr, System.Globalization.NumberStyles.HexNumber);
            int r= int.Parse(Rstr, System.Globalization.NumberStyles.HexNumber);
            int g= int.Parse(Gstr, System.Globalization.NumberStyles.HexNumber);
            int b= int.Parse(Bstr, System.Globalization.NumberStyles.HexNumber);
            Bitmap bmp = new Bitmap(x, y);
            using (Graphics graph = Graphics.FromImage(bmp))
            {
                Rectangle ImageSize = new Rectangle(0, 0, x, y);
                Brush brus = new SolidBrush(Color.FromArgb(a,r,g,b));
                graph.FillRectangle(brus, ImageSize);
            }
            return bmp;
        }
        ...
        public static Bitmap BitmapFromDataUri(string dataUri)
        {

            Bitmap Image = null;
            var binData = UriDataToBytesArray(dataUri);


            Image = BitmapFromBytesArray(binData);

            return Image;
        }

        public static byte[] UriDataToBytesArray(string dataUri)
        {
            var base64Data = UriDataToBase64String(dataUri);
        ...
        public static string UriDataToBase64String(string dataUri)
        {
            var base64Data = Regex.Match(dataUri, @"data:image/(?<type>.+?),(?<data>.+)").Groups["data"].Value;
            return base64Data;
        }

        public static Bitmap BitmapFromBytesArray(byte[] binData)
        {
            Bitmap Image;
        ...
        public static string SavePostedImageFile(Stream postedFile, string path, string mimeType, string originalFileName, int quality = 100, bool isResize = true)
        {
            Bitmap bitmap;
            bitmap = new Bitmap(postedFile);
            int maxWidth = 2000;
            int maxResizableWidth = 4000;
            string extension = originalFileName.Substring(originalFileName.LastIndexOf('.'));

            if (isResize && bitmap.Width > maxWidth && bitmap.Width <= maxResizableWidth)
            {
                double height = (int)(bitmap.AspectRatio() * maxWidth);
                bitmap = Resizer.ResizeImage(bitmap, null, maxWidth.ToString(), height.ToString(), FitMode.Max);
            }
