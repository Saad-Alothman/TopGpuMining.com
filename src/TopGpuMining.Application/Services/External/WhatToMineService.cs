...
namespace TopGpuMining.Application.Services
{
    public static class WhatToMineService
    {
        public static List<Coin> GetAllCoinsFromCalculators()
        {
            ...
            foreach (JToken property in coinsjson.Children())
            {
                ...
                WhatToMineCoinResponse whatToMineCoinResponse = Newtonsoft.Json.JsonConvert.DeserializeObject<WhatToMineCoinResponse>(values);
        ...
        public static Coin GetCoinInfo(int coinWhatToMineId)
        {
            ...
            try
            {
                ...
                //  var values = CreaDev.Framework.Core.Utils.Serialization.DeSerialize<Dictionary<string, string>>(response);
                WhatToMineCoinResponse whatToMineCoinResponse = Newtonsoft.Json.JsonConvert.DeserializeObject<WhatToMineCoinResponse>(response);
    ...
    public class WhatToMineCoinResponse
    {
        ...
        public Coin ToCoin(string name)
        {
            ...
            coin.Tag = this.tag;
            IAlgorithmService algorithmService = ServiceLocator.Current.GetService<IAlgorithmService>();
            var algorithmInDb = algorithmService.AddOrUpdate(new Algorithm() { Name = new Core.Entities.LocaleString(algorithm, algorithm) });
